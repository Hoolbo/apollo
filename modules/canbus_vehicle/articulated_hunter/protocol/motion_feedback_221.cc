...
void Motionfeedback221::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_motion_feedback_221()->set_linear_speed(linear_speed(bytes, length));
  chassis->mutable_motion_feedback_221()->set_steering_angle(steering_angle(bytes, length));
}
...
double Motionfeedback221::linear_speed(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
